...
// The lesson is now built one small scene per beat (sequential calls), so each
...
function layoutAndLint(scene: SceneSpec, beat: TeachingBeat, prev?: SceneSpec) {
  const patterned = applyShotPattern(scene, beat);
  const laidOut = resolveLayout(patterned, { previousScene: prev });
...
function layoutOfflineLesson(lesson: Lesson, script: LessonScript): Lesson {
...
async function buildLesson(script: LessonScript, send: (event: LessonStreamEvent) => void) {
